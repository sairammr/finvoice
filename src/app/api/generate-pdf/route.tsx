...
const s = StyleSheet.create({
  ...
  logoHeds:     { fontSize: 26, fontFamily: "Helvetica-Bold", color: C.ink,      letterSpacing: -0.5 },
  logoUp:      { fontSize: 26, fontFamily: "Helvetica-Bold", color: C.limeDark, letterSpacing: -0.5 },
...
function InvoicePDF({ invoice }: { invoice: any }) {
  ...
  return (
    <Document title={`Invoice ${invoice.id}`} author="Finvoice" subject="Private Invoice Factoring">
      <Page size="A4" style={s.page}>
        ...
        <View style={s.footer} fixed>
          <Text style={s.footerBrand}>Generated by Finvoice — Private Invoice Factoring on Flare</Text>
...
export async function GET(request: NextRequest) {
  ...
  const subtle   = rgb(0.62, 0.65, 0.70);  // #9ca3af
  ...
  page.drawText("Step 2: enter the code in the OTP field and click VERIFY.  Step 3: click APPROVE ON FLARE.", {
  ...
  btnCheck.setAlignment(1 as any);
  ...
  btnVerify.setAlignment(1 as any);
  ...
  btnApprove.setText("Approve on Flare");
  btnApprove.enableReadOnly();
  btnApprove.setAlignment(1 as any);
...
function doVerify() {
  ...
  var sf = this.getField("status");
  if (sf) sf.value = "Verifying OTP... wait for confirmation, then click Approve on Flare.";
  try {
...
function doApprove() {
  var sf = this.getField("status");
  if (sf) sf.value = "Submitting approval to Flare... check dashboard for confirmation.";
  try {
...
`;
  pdfDoc.addJavaScript("finvoice", js);
